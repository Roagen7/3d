...
TestScene::TestScene() {
    ...
    std::cout << ob4.triangles[0].q[0].x << std::endl;
...
void TestScene::updateProperties(std::vector<sf::Keyboard::Key> keysPressed, sf::Vector2<double> mouseDelta) {
    ...
    this->theta += 0.01;
    this->meshes[3].localTransform = Matrix::getRotationMatrixAxisX(3.14) * Matrix::getScaleMatrix({0.01,0.01,0.01});
